...
inline void G_Vec(int u,int fa,int last,Vec &Sum,Vec &Up_Sum,Vec &num){
...
inline void Div(int u){
	...
	G_Vec(u,0,u,tmp1,tmp2,tmp3);
	std::sort(tmp1.begin(),tmp1.end());
	std::sort(tmp2.begin(),tmp2.end());
    std::sort(tmp3.begin(),tmp3.end());
    tmp1.pb(mk(A,0)); tmp2.pb(mk(A,0)); tmp3.pb(mk(A,0));
    int tmp = -1, tmp_Sum = 0;
    for (D it = tmp1.begin(); it != tmp1.end(); ++ it)
        if (it -> fi != tmp) Sum[u].pb(mk(tmp,tmp_Sum)), tmp = it -> fi, tmp_Sum = it -> se;
        else tmp_Sum += it -> se;
    tmp = -1, tmp_Sum = 0;
    for (D it = tmp2.begin(); it != tmp2.end(); ++ it)
        if (it -> fi != tmp) Up_Sum[u].pb(mk(tmp,tmp_Sum)), tmp = it -> fi, tmp_Sum = it -> se;
        else tmp_Sum += it -> se;
    tmp = -1, tmp_Sum = 0;
    for (D it = tmp3.begin(); it != tmp3.end(); ++ it)
        if (it -> fi != tmp) num[u].pb(mk(tmp,tmp_Sum)), tmp = it -> fi, tmp_Sum = it -> se;
        else tmp_Sum += it -> se;
    for (int i = 1; i < Sum[u].size(); i++) Sum[u][i].se += Sum[u][i-1].se;
    for (int i = 1; i < Up_Sum[u].size(); i++) Up_Sum[u][i].se += Up_Sum[u][i-1].se;
    for (int i = 1; i < num[u].size(); i++) num[u][i].se += num[u][i-1].se;
	for (int i = first[u]; i; i = a[i].next){
...
inline int Query(int l,int r,Vec &Sum){
...
int T_Sum[MAXN],T_num[MAXN],T_Up_Sum[MAXN];

inline int Query(int x,int L,int R){
	int Ans = 0;
	for (int u = x; u; u = pre[u]) T_Sum[u] = Query(L,R,Sum[u]), T_num[u] = Query(L,R,num[u]), T_Up_Sum[u] = Query(L,R,Up_Sum[u]);
