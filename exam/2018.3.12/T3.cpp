...
int n, m, val[MAXN], Ans[MAXN * 10], root[MAXN], cnt, first[MAXN], Mn, Mx, e, T[MAXN];  
...
inline void push(RG int u, RG int v, RG int w) {
...
inline void Work() {
    ...
    for (int i = n; i; --i) {
        last = 0;
        Update(root[i], root[i + 1], Mn, Mx, val[i], i);
        while (true) {
    ...
    for (int i = m; i; --i) {
        ...
        gmin(Ans[Q[i].id], Qu(Q[i].r));
    }
}
 
int main() {
    n = read(); val[0] = inf; Mn = inf, Mx = -inf; 
    for (int i = 1; i <= n; ++i) val[i] = read(), gmin(Mn, val[i]), gmax(Mx, val[i]);
    m = read();
    for (int i = 1; i <= m; ++i) Q[i].id = i, Q[i].l = read(), Q[i].r = read(), Ans[i] = inf; 
    Work();
    std::reverse(&val[1], &val[n + 1]);
    for (int i = 1; i <= m; ++i) Q[i].l = n - Q[i].l + 1, Q[i].r = n - Q[i].r + 1, std::swap(Q[i].l, Q[i].r);
    Work();
