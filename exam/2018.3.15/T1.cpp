...
const int MAXN = 1005; 
const int mod = 1e9 + 7;
int fac[MAXN * MAXN], Inv[MAXN * MAXN], n, m, Gcd[MAXN][MAXN], tot, Sgcd[MAXN][MAXN], Sum[MAXN][MAXN], Sum_Gcd[MAXN][MAXN], f[MAXN][MAXN]; 
...
inline int Count() {
	RG int Ans = 0, S, cnt, tmp, last = 0, t1, t2, QAQ, lim, i, j, k;
	for (i = 1; i <= n; ++i)
		for (j = 1; j <= m; ++j) {
			if (f[i][j]) {
			...
			for (k = 0; k <= i; ++k) {
				QAQ = k * j, lim = QAQ / i;
				if (lim * i == QAQ) -- lim; 
				t1 = 1ll * k * j * (lim + 1) - 1ll * i * lim * (lim + 1) / 2;
				t2 = ((1ll * Gcd[i][j] * (lim + 1) % mod + Sum_Gcd[k][lim]) % mod + ((Sum_Gcd[i - k][j] - ((j == lim) ? 0 : Sum_Gcd[i - k][j - lim - 1]) + mod) % mod)) % mod; 
				upd(tmp, 2ll * (((t1 - t2 + 2ll * (lim + 1) % mod) % mod + mod) % mod) % mod);
			}
...
int main() {
	...
	for (i = 0; i <= n; ++i) Gcd[i][0] = Gcd[0][i] = i;
	...
	for (i = 1; i <= n; ++i)
		for (j = 1; j <= n; ++j)
			upd(Sgcd[i][j], Sgcd[i][j - 1] + Gcd[i][j]), upd(Sum[i][j], Sum[i][j - 1] + i * j % mod);
