# include <bits/stdc++.h> 
const int N = 55; 
int n, m, mod, gcd[N][N], Ans, size[N], Inv[N], fac[N]; 
...
inline void dfs(int left, int last, int cnt, int id, int res) {
    if (!left) { 
    ...
    for (int i = last; i <= left; ++i)
        size[id] = i, dfs(left - i, i, last == i ? cnt + 1 : 1, id + 1, 1ll * res * fac[i - 1] % mod * C[left][i] % mod);
}
